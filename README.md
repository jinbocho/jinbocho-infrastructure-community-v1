...
| `JINBOCHO_FEATURES` | `catalog,auth,kids` | No | Must match `api-gateway`'s value. `kids` (Kids Mode) is free and on by default. Add `social` for cross-library social (ADR-019/020), off by default — install-time only, see ADR-021 |
...
| `JINBOCHO_FEATURES` | `catalog,auth,kids` | No | Comma-separated enabled modules. `kids` (Kids Mode) is free and on by default. Add `social` for cross-library social (ADR-019/020), off by default — install-time only, see ADR-021 |
...
## 6. Observability (Netdata)

Optional and **off by default** — skip this section and the stack behaves
exactly as described above. Single container, no external account, no
secrets to configure — host + per-container metrics (CPU/RAM/disk/network)
with a built-in dashboard.

### 6.1 Start the stack with the profile enabled
...
It binds to `127.0.0.1:19999` only — never exposed publicly. From your own
machine, tunnel in:
...
Per-container metrics require mounting `/var/run/docker.sock` (read-only)
into the Netdata container. `:ro` only makes the socket *file* read-only —
it does not restrict what the Docker API itself allows over that socket, so
this effectively grants Netdata the same power as a member of the host's
`docker` group (root-equivalent). This is a reasonable trade-off on a
single-operator VPS with the dashboard bound to localhost only; if it isn't
